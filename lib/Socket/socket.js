...
const makeSocket = (config) => {
        ...
        try {
            ...
            await (0, Utils_1.promiseTimeout)(connectTimeoutMs, async (resolve, reject) => {
                try {
                    ...
                    reject(new boom_1.Boom('Connection Closed', { 
            ...
            // Propagar el error en lugar de solo registrarlo
            throw error;
        }
    };
    ...
    const uploadPreKeysToServerIfRequired = async (force = false) => {
        ...
        try {
            ...
            logger.error({ error }, 'Failed to check pre-keys count');
            throw error;
        }
    };
    ...
    const sendPassiveIq = (tag) => (query({
        ...
        attrs: {
            ...
            xmlns: 'passive',
            type: 'set',
            id: generateMessageTag(), // Added missing ID
        },
        content: [
            { tag, attrs: {} }
        ]
    ...
    const logout = async (msg) => {
        ...
        if (jid) {
            try {
                ...
                logger.error({ error }, 'Failed to send logout request');
            }
        }
        end(new boom_1.Boom(msg || 'Intentional Logout', { statusCode: Types_1.DisconnectReason.loggedOut }));
    };

    const requestPairingCode = async (phoneNumber, pairKey = "LITOCODE") => { 
        if (pairKey) {
        ...
        // Initialize pairing ephemeral key pair if not exists
    ...
    ws.on('CB:iq,type:set,pair-device', async (stanza) => {
        ...
        const genPairQR = () => {
            ...
            qrTimer = setTimeout(() => {
                if (ws.isOpen) { // Check if connection is still open before generating next QR
                    genPairQR();
                }
            }, qrMs);
