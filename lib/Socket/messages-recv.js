...
// Funciones auxiliares para verificar tipos de JID
...
export const makeMessagesRecvSocket = (config) => {
        ...
        try {
            await processingMutex.mutex(async () => {
                ...
                if (typeof status !== 'undefined' &&
                    ...
                    (status >= WAProto.WebMessageInfo.Status.SERVER_ACK || !isNodeFromMe)) {
                    if (isJidGroup(remoteJid) || isJidStatusBroadcast(remoteJid)) {
                        if (attrs.participant) {
                            const updateKey = status === WAProto.WebMessageInfo.Status.DELIVERY_ACK ? 'receiptTimestamp' : 'readTimestamp';
    ...
    const handleMessage = async (node) => {
        ...
        if (msg.message?.protocolMessage?.type === WAProto.Message.ProtocolMessage.Type.SHARE_PHONE_NUMBER &&
            ...
            case 'message':
                ...
                if (plaintextNode?.content) {
                    try {
                        ...
                        const messageProto = WAProto.Message.decode(contentBuf);
