...
// Sistema de deduplicación para evitar envíos múltiples
const messageProcessCache = new node_cache_1.default({
    stdTTL: 60, // 1 minuto
    useClones: false
});
...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        if (messageProcessCache.get(msgId)) {
            logger.debug({ msgId }, 'Message already being processed, skipping duplicate');
            return msgId;
        }
        
        // Marcar este mensaje como en proceso
        messageProcessCache.set(msgId, true);
        ...
        try {
            ...
            // Si hay un error, eliminar de la caché para permitir reintentos
            messageProcessCache.del(msgId);
            throw error;
        }
    };
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // Generar un ID único para esta operación de envío
            const operationId = options.messageId || (0, Utils_1.generateMessageIDV2)((_c = sock.user) === null || _c === void 0 ? void 0 : _c.id);
            
            // Verificar si esta operación ya se procesó
            if (messageProcessCache.get(operationId)) {
                logger.debug({ operationId }, 'Message send operation already processed, skipping duplicate');
                // Devolver un mensaje simulado para no romper el flujo
                return {
                    key: {
                        remoteJid: jid,
                        fromMe: true,
            ...
            // Marcar esta operación como procesada
            messageProcessCache.set(operationId, true);
            ...
            try {
                ...
                if (typeof content === 'object' && 'album' in content && content.album) {
                    ...
                    const fullMsg = await (0, Utils_1.generateWAMessage)(jid, content, {
                        ...
                        messageId: operationId, // Usar el operationId como messageId
                        ...options,
                    });
                    ...
                    try {
                        ...
                        // Si hay un error al enviar, eliminar de la caché para permitir reintentos
                        messageProcessCache.del(operationId);
                        throw error;
                    }
                }
            } catch (error) {
                // Si hay un error en el proceso general, eliminar de la caché para permitir reintentos
                messageProcessCache.del(operationId);
                throw error;
            }
        }
