...
// Sistema de deduplicación para evitar generación múltiple de mensajes
const messageGenerationCache = new Map();
const CACHE_TTL = 5000; // 5 segundos

// Función para generar una clave de caché única para cada mensaje
const generateCacheKey = (jid, content, options) => {
    const contentStr = JSON.stringify(content);
    const optionsStr = JSON.stringify({ 
        ephemeralExpiration: options.ephemeralExpiration,
        timestamp: options.timestamp,
        quoted: options.quoted?.key?.id
    });
    return `${jid}:${contentStr}:${optionsStr}`;
};

// Función para verificar si un mensaje ya está en caché
const isMessageInCache = (cacheKey) => {
    const cached = messageGenerationCache.get(cacheKey);
...
// Función para agregar un mensaje a la caché
const addMessageToCache = (cacheKey) => {
    messageGenerationCache.set(cacheKey, { timestamp: Date.now() });
    
    // Limpiar entradas expiradas
    if (messageGenerationCache.size > 100) {
        const now = Date.now();
        for (const [key, value] of messageGenerationCache.entries()) {
            if (now - value.timestamp > CACHE_TTL) {
                messageGenerationCache.delete(key);
            }
        }
    }
};
...
const generateWAMessage = async (jid, content, options) => {
    ...
    // Generar una clave de caché única para este mensaje
    const cacheKey = generateCacheKey(jid, content, options);
    
    // Verificar si este mensaje ya está siendo procesado
    if (isMessageInCache(cacheKey)) {
        // Si ya está en caché, continuar con el procesamiento normal pero no agregar a la caché nuevamente
        return (0, exports.generateWAMessageFromContent)(jid, await (0, exports.generateWAMessageContent)(content, { newsletter: (0, WABinary_1.isJidNewsletter)(jid), ...options }), options);
    }
    
    // Agregar a la caché para evitar procesamiento duplicado
    addMessageToCache(cacheKey);
...
const downloadMediaMessage = async (message, type, options, ctx) => {
    ...
    async function downloadMsg() {
        ...
        if (!media || typeof media !== 'object' || (!('url' in media) && !('thumbnailDirectPath' in media))) {
            throw new boom_1.Boom(`"${contentType}" message is not a media message`, { statusCode: 400, data: message });
        }
        let download;
